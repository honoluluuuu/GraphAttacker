...
# costruzione del modello con layers generici
        ...
        self.gen_fea = tf.matmul(self.d_fea, self.gen_variable['weight']) + self.gen_variable['bias'] #self.diffpool.d_fea
        self.gen_fea = self.replace(self.gen_fea,self.fea_dense, self.att_size)
        # self.gen_fea = tf.sigmoid(self.gen_fea)

        self.maxfea = tf.transpose(tf.multiply(tf.ones([90,1]),tf.expand_dims(tf.reduce_max(self.gen_fea, 1),0)))
        self.gen_fea_L = tf.sign(tf.nn.relu(self.gen_fea-self.maxfea+0.0000001))
        self.gen_fea_L = self.replace(self.gen_fea_L, self.fea_dense, self.att_size)

        self.gen_fea_G = tf.reshape(self.gen_fea, [-1, self.num_nodes * self.input_dim])
        # self.a = tf.multiply(self.one,self.m)

        # #
        self.pre= self.diffpool.predict(self.adj, self.gen_fea, self.pool_mat, self.num_features_nonzero, self.num_nodes, self.assign_dim, self.hidden_dim, self.embedding_dim,
                                         self.nomalize_adj, self.diffpool.gcn_layers, self.diffpool.pool_layers, self.dropout)  #, self.w0, self.w1, self.w2, self.w3

        self.pre_L= self.diffpool.predict(self.adj, self.gen_fea_L, self.pool_mat, self.num_features_nonzero, self.num_nodes, self.assign_dim, self.hidden_dim, self.embedding_dim,
                                         self.nomalize_adj, self.diffpool.gcn_layers, self.diffpool.pool_layers, self.dropout)  #, self.w0, self.w1, self.w2, self.w3
...
class GraphAttack(AttackNet):
    def __init__(self, placeholders,diffpool, input_dim, featureless,idx, num_graphs, num_nodes,att_size, assign_dim, with_pooling, **kwargs):
        ...
        self.fea_dense = placeholders['att_fea']
        ...
        self.fea_G = tf.reshape(self.fea_dense, [-1, self.num_nodes * self.input_dim])
    ...
    def _loss(self):
        ...
        self.loss += masked_cross_entropy(self.output_ori, self.placeholders['labels'],
                                                  self.placeholders['labels_mask'])
        self.D_real, self.D_logit_real = self.discriminator(self.fea_G)  # 取得判别器判别的真实手写数字的结果
        self.D_fake, self.D_logit_fake = self.discriminator(self.gen_fea_G)  # 取得判别器判别的生成的手写数字的结
        ...
        self.G_loss = tf.reduce_mean(tf.square(self.fea_G - self.gen_fea_G))
    ...
    #
    def _accuracy(self):
        self.accuracy_att = masked_accuracy(self.pre_L, self.placeholders['labels'],
                                        self.placeholders['labels_mask'])
    ...
    def _build_dis(self):
        self.dis_variable = {
            'D_w1': tf.Variable(tf.random_normal([self.num_nodes * self.input_dim, self.num_hidden])),
    ...
    def _build_gen(self):
        self.gen_variable = {
            'weight': tf.Variable(tf.random_normal([FLAGS.hidden2, self.input_dim])),
            'bias': tf.Variable(tf.random_normal([self.num_nodes, self.input_dim]))}
    # def predict(self):
