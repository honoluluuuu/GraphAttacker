...
# costruzione del modello con layers generici
        ...
        self.d_fea = self.diffpool.gcn_layers[0](self.inputs, self.support, self.num_features_nonzero, self.num_nodes)
        ...
        self.gen_fea = tf.matmul(self.d_fea, self.gen_variable_X['weight_X']) + self.gen_variable_X['bias_X'] #self.diffpool.d_fea
        self.gen_fea = self.replace_X(self.gen_fea,self.fea, self.att_size)
        # self.gen_fea = tf.sigmoid(self.gen_fea)

        self.maxfea = tf.transpose(tf.multiply(tf.ones([26,1]),tf.expand_dims(tf.reduce_max(self.gen_fea, 1),0)))
        self.gen_fea_L = tf.sign(tf.nn.relu(self.gen_fea-self.maxfea+0.0000001))
        self.gen_fea_L = self.replace_X(self.gen_fea_L, self.fea, self.att_size)

        self.gen_fea_G = tf.reshape(self.gen_fea, [-1, self.num_nodes * self.input_dim])


        # #
        self.pre= self.diffpool.predict(self.gen_adj, self.inputs, self.pool_mat, self.num_features_nonzero, self.num_nodes, self.assign_dim, self.hidden_dim, self.embedding_dim,
                                         self.nomalize_adj, self.diffpool.gcn_layers, self.diffpool.pool_layers, self.dropout)  #, self.w0, self.w1, self.w2, self.w3

        self.output_ori = self.diffpool.predict(self.adj, self.inputs, self.pool_mat, self.num_features_nonzero, self.num_nodes, self.assign_dim, self.hidden_dim, self.embedding_dim,
                                         self.nomalize_adj, self.diffpool.gcn_layers, self.diffpool.pool_layers, self.dropout)  #,self.v0, self.v1, self.v2, self.v3

        self.pre_X= self.diffpool.predict(self.gen_adj, self.gen_fea, self.pool_mat, self.num_features_nonzero, self.num_nodes, self.assign_dim, self.hidden_dim, self.embedding_dim,
        ...
        variables_gen_X = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='gen_X')
        self.vars_gen_X = {var.name: var for var in variables_gen_X}
# inizializzo loss e accuracy
        ...
        self.C_op = tf.train.AdamOptimizer(learning_rate=0.033).minimize(self.C_loss, var_list=self.vars_gen)  # 生成器的训练器

        self.D_op_x = tf.train.AdamOptimizer().minimize(self.D_loss_x, var_list=self.vars_dis)  # 判别器的训练器
        self.G_op_x = tf.train.AdamOptimizer(learning_rate=0.03).minimize(self.G_loss_x, var_list=self.vars_gen_X)  # 生成器的训练器
        self.C_op_x = tf.train.AdamOptimizer(learning_rate=0.033).minimize(self.C_loss_x, var_list=self.vars_gen_X)  # 生成器的训练器
    ...
    def replace_X(self, gen_adj, ori_adj, dim):
        o_r0, o_r1 = tf.split(ori_adj,[dim, self.num_nodes - dim], 0)
        g_r0, g_r1 = tf.split(gen_adj, [dim, self.num_nodes - dim], 0)


        fea = tf.concat([g_r0,o_r1],0)
        return fea
    ...
    def discriminator_X(self,x):
        D_h1 = tf.nn.relu(tf.matmul(x, self.dis_variable['D_w1_X']) + self.dis_variable['D_b1_X'])  # 输入乘以D_W1矩阵加上偏置D_b1，D_h1维度为[N, 128]
        D_logit = tf.matmul(D_h1, self.dis_variable['D_w2_X']) + self.dis_variable['D_b2_X']  # D_h1乘以D_W2矩阵加上偏置D_b2，D_logit维度为[N, 1]
        D_prob = tf.nn.sigmoid(D_logit)  # D_logit经过一个sigmoid函数，D_prob维度为[N, 1]
...
class GraphAttack(AttackNet):
    def __init__(self, placeholders,diffpool, input_dim, featureless,idx, num_graphs, num_nodes,att_size, assign_dim, with_pooling, **kwargs):
        ...
        self.support = placeholders['att_adj']
        self.adj_G = tf.reshape(self.adj, [-1, self.num_nodes * self.num_nodes])
        self.adj_G = tf.cast(self.adj_G, dtype=tf.float32)

        self.fea_G = tf.reshape(self.fea, [-1, self.num_nodes * self.input_dim])
        self.fea_G = tf.cast(self.fea_G, dtype=tf.float32)
   ...
   #X
        self.D_real_X, self.D_logit_real_X = self.discriminator_X(self.fea_G)  # 取得判别器判别的真实手写数字的结果
        self.D_fake_X, self.D_logit_fake_X = self.discriminator_X(self.gen_fea_G)  # 取得判别器判别的生成的手写数字的结

        self.D_loss_real_X = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(logits=self.D_logit_real_X, labels=tf.ones_like(
                self.D_logit_real_X)))  # 对判别器对真实样本的判别结果计算误差(将结果与1比较)
        self.D_loss_fake_X = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(logits=self.D_logit_fake_X, labels=tf.zeros_like(
                self.D_logit_fake_X)))  # 对判别器对虚假样本(即生成器生成的手写数字)的判别结果计算误差(将结果与0比较)

        self.D_loss_x = self.D_loss_real_X + self.D_loss_fake_X  # 判别器的误差
        self.G_loss_x = self.rate * tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.D_logit_fake_X,
                                                                                           labels=tf.ones_like(
                                                                                               self.D_logit_fake_X))) \
                        + tf.reduce_mean(tf.square(self.fea_G - self.gen_fea_G))  # 生成器的误差(将判别器返回的对虚假样本的判别结果与1比较)
        self.C_loss_x = masked_cross_entropy(self.pre_X, self.placeholders['labels'],
    ...
    def _build_dis(self):
        self.dis_variable = {
            ...
            'D_w1_X': tf.Variable(tf.random_normal([self.num_nodes * self.input_dim, self.num_hidden])),
            'D_b1_X': tf.Variable(tf.random_normal([self.num_hidden])),
            'D_w2_X': tf.Variable(tf.random_normal([self.num_hidden, 1])),
            'D_b2_X': tf.Variable(tf.random_normal([1])),
        }
    ...
    def _build_gen_X(self):
        self.gen_variable_X = {
            'weight_X': tf.Variable(tf.random_normal([FLAGS.hidden2, self.input_dim])),
            'bias_X': tf.Variable(tf.random_normal([self.num_nodes, self.input_dim])),
        }
...
# costruzione del modello con layers generici
        ...
        out_all_f = []
        hidden1 = self.gcn_layers[0](self.activations_f[-1], self.att_adj, self.att_num_features_nonzero, self.num_nodes)  # num_nodes*hidden_dim
        self.x_all_f = [hidden1]
        self.activations_f.append(hidden1)

        hidden2 = self.gcn_layers[1](self.activations_f[-1], self.att_adj, self.att_num_features_nonzero, self.num_nodes)  # num_nodes*hidden_dim
        self.x_all_f.append(hidden2)
        self.activations_f.append(hidden2)

        hidden3 = self.gcn_layers[2](self.activations_f[-1], self.att_adj, self.att_num_features_nonzero, self.num_nodes)  # num_nodes*embedding_dim
        ...
        self.assign_tensor_f = self.pool_layers[0](self.activations_pool_f[-1], self.att_adj,
                                                   self.att_num_features_nonzero, self.num_nodes)
        ...
        hidden21 = self.gcn_layers[3](self.activations_f[-1], self.assign_a_f, self.att_num_features_nonzero, self.assign_dim)
        self.x_all_2_f = [hidden21]
        self.activations_f.append(hidden21)

        hidden22 = self.gcn_layers[4](self.activations_f[-1], self.assign_a_f, self.att_num_features_nonzero, self.assign_dim)
        self.x_all_2_f.append(hidden22)
        self.activations_f.append(hidden22)

        hidden23 = self.gcn_layers[5](self.activations_f[-1], self.assign_a_f, self.att_num_features_nonzero, self.assign_dim)
        ...
        for i in range(len(self.support)):
            ...
            #第一个gcn块
            hidden1 = self.gcn_layers[0](self.inputs[i], self.support[i], self.num_features_nonzero[i], self.num_nodes)
            x_all = [hidden1]
            self.activations[i].append(hidden1)

            hidden2 = self.gcn_layers[1](hidden1, self.support[i], self.num_features_nonzero[i], self.num_nodes)
            x_all.append(hidden2)
            self.activations[i].append(hidden2)

            hidden3 = self.gcn_layers[2](hidden2, self.support[i], self.num_features_nonzero[i], self.num_nodes)
            ...
            # 第一个pool块
            self.assign_tensor =self.pool_layers[0](self.activations_pool[i][-1], self.support[i], self.num_features_nonzero[i], self.num_nodes)
            ...
            self.assign_tensor = self.pool_layers[1](self.activations_pool[i][-1], self.support[i],
                                                     self.num_features_nonzero[i], self.num_nodes)
            ...
            hidden21 = self.gcn_layers[3](self.activations[i][-1], self.assign_a, self.num_features_nonzero[i], self.assign_dim)
            self.x_all_2 = [hidden21]
            self.activations[i].append(hidden21)

            hidden22 = self.gcn_layers[4](self.activations[i][-1], self.assign_a, self.num_features_nonzero[i], self.assign_dim)
            self.x_all_2.append(hidden22)
            self.activations[i].append(hidden22)

            hidden23 = self.gcn_layers[5](self.activations[i][-1], self.assign_a, self.num_features_nonzero[i], self.assign_dim)
...
# inizializzo loss e accuracy
    ...
    def predict(self, adj, input, pool_mat, num_features_nonzero, num_nodes, assign_dim, hidden_dim, embedding_dim, nomalize_adj, gcn_layers, pool_layers, dropout):
        out_all_f = []
        hidden1 = gcn_layers[0](input, adj, num_features_nonzero, self.num_nodes)  # num_nodes*hidden_dim
        x_all_f = [hidden1]
        # self.activations_f.append(hidden1)

        hidden2 = gcn_layers[1](hidden1, adj, num_features_nonzero, self.num_nodes)  # num_nodes*hidden_dim
        x_all_f.append(hidden2)
        # self.activations_f.append(hidden2)

        hidden3 = gcn_layers[2](hidden2, adj, num_features_nonzero, self.num_nodes)  # num_nodes*embedding_dim
        x_all_f.append(hidden3)

        x_tensor_f = tf.multiply(tf.concat(x_all_f, axis=1), pool_mat)
        out_1_f = tf.reduce_max(x_tensor_f, 1)
        out_all_f.append(out_1_f)

        # 第一个pool块
        assign_tensor_f = pool_layers[0](input, adj, num_features_nonzero, self.num_nodes)
        pool_all_f = [assign_tensor_f]
        assign_tensor_f = pool_layers[1](assign_tensor_f, adj, num_features_nonzero, self.num_nodes)
        pool_all_f.append(assign_tensor_f)
        assign_tensor_f = pool_layers[2](assign_tensor_f, adj, num_features_nonzero, self.num_nodes)
        pool_all_f.append(assign_tensor_f)

        assign_tensor_f = tf.nn.softmax(
            tf.layers.dense(tf.concat(pool_all_f, axis=1), assign_dim, name='pool', reuse=True), axis=-1)

        x_f = tf.matmul(tf.transpose(assign_tensor_f),x_tensor_f)
        assign_a_f = tf.matmul(tf.matmul(tf.transpose(assign_tensor_f), adj), assign_tensor_f)

        # 第二个gcn块
        # self.activations_f.append(self.x_f)
        hidden21 = gcn_layers[3](x_f, assign_a_f, num_features_nonzero, self.assign_dim)
        x_all_2_f = [hidden21]


        hidden22 = gcn_layers[4](hidden21, assign_a_f, num_features_nonzero, self.assign_dim)
        x_all_2_f.append(hidden22)

        hidden23 = gcn_layers[5](hidden22, assign_a_f, num_features_nonzero, self.assign_dim)
        x_all_2_f.append(hidden23)
        x_tensor2_f = tf.concat(x_all_2_f, axis=1)
        out_2_f = tf.reduce_max(x_tensor2_f, 1)
        out_all_f.append(out_2_f)
        output = tf.expand_dims(tf.concat(out_all_f, axis=0), 0)
