...
from Xnet import GCNGraphs,GraphAttack
# Set random seed
import numpy as np
import copy
import random
...
flags.DEFINE_integer('epochs_c', 12, 'Number of epochs to train.')
...
while(attack_num<100):
    ...
    att_success = 0
    gcn_acc = 0
    save_dir = "result/DD_result-X-"+str(budget)+"-onehot.txt"
    ...
    while (att_success == 0 and att_time < 10):
        ...
        adj_save = 'DD/adj_DD-X-'+str(budget)+'-onehot/gen_DD_' + str(attack_graph) + '_' + str(att_time)
        ...
        #         i, graph, fea, labels, graph_size, a_size)
        ...
        while (epoch < FLAGS.epochs_gan):
            ...
            for g_epoch in range(FLAGS.epochs_g):
                ...
                out_gen = sess.run([Attgraph.G_op, Attgraph.G_loss, Attgraph.gen_fea, Attgraph.maxfea, Attgraph.gen_fea_L], feed_dict=Att_dict)
            ...
            for c_epoch in range(FLAGS.epochs_c):
                ...
                out_cla = sess.run([Attgraph.C_op, Attgraph.C_loss, Attgraph.gen_fea, Attgraph.pre_L, Attgraph.gen_fea_L], feed_dict=Att_dict)
            ...
            # eval_c = eval_X_l2(att_fea_dense[:att_size], fea_C[:att_size])
            eval_c = eval_X_onehot(att_fea_dense[:att_size], fea_C[:att_size])
    ...
    print("第", attack_graph, "个节点攻击，图节点数：", att_size, "，属性数：", att_size, "，平均修改数:", "{:.3f}".format(modify_N),
          ",  节点原类标为", ori_class,
    ...
    f.write(str(attack_graph) + " " + str(gcn_acc) + " " + str(acc_node) + " " + str(modify_N) + " " + " 第" + str(
        attack_graph) + "个图攻击，图节点数：" + str(node_num) + "，属性数：" + str(att_size) + "，平均修改数:" + str(
        "{:.3f}".format(modify_N)) +
