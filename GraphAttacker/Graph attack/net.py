...
# costruzione del modello con layers generici
        ...
        # #
        self.pre = self.diffpool.predict(self.gen_adj_L, self.inputs, self.pool_mat, self.num_features_nonzero, self.num_nodes, self.assign_dim,
                                         self.nomalize_adj, self.diffpool.gcn_layers, self.diffpool.pool_layers,
                                         self.diffpool.dis_variable,self.dropout)
        #
        self.output_ori = self.diffpool.predict(self.adj, self.inputs, self.pool_mat, self.num_features_nonzero, self.num_nodes, self.assign_dim,
                                         self.nomalize_adj, self.diffpool.gcn_layers, self.diffpool.pool_layers,
                                         self.diffpool.dis_variable,self.dropout)
...
# inizializzo loss e accuracy
    ...
    # def prediction1(self, support, input, pool_mat, layer_gcn_1, layer_pool_1, num_features_nonzero, dropout):
    #
    #     In = tf.constant(1, shape=[self.num_nodes], dtype=tf.float32)
    #     support1, D, D1 = self.nomalize_adj(tf.add(support, tf.diag(In)), self.num_nodes)    #规范化的A
    #     out_all = []
    #     # self.activations.append(self.inputs)
    #     #  第一个gcn块
    #     input_gcn_1_0 = sparse_dropout(input, 1 - dropout, num_features_nonzero)
    #     pre_sup = tf.sparse_tensor_dense_matmul(input_gcn_1_0, layer_gcn_1[0].weights['weights_0'])
    #     output1 = tf.nn.relu(tf.matmul(support1, pre_sup) + layer_gcn_1[0].weights['bias'])
    #     x_all = [output1]
    #
    #     input_1_1 = tf.nn.dropout(output1, dropout)
    #     pre_sup_2 = tf.matmul(input_1_1, layer_gcn_1[1].weights['weights_0'])
    #     output2 = tf.nn.relu(tf.matmul(support1, pre_sup_2) + layer_gcn_1[1].weights['bias'])
    #     x_all.append(output2)
    #
    #     input_1_2 = tf.nn.dropout(output2, dropout)
    #     pre_sup_3 = tf.matmul(input_1_2, layer_gcn_1[2].weights['weights_0'])
    #     output3 = tf.matmul(support1, pre_sup_3) + layer_gcn_1[2].weights['bias']
    ...
    #     pre_sup_pool_0 = tf.sparse_tensor_dense_matmul(input_gcn_2_0, layer_pool_1[0].weights['weights_0'])
    #     output_pool0 = tf.nn.relu(tf.matmul(support1, pre_sup_pool_0) + layer_pool_1[0].weights['bias'])
    #     pool_all = [output_pool0]
    #
    #     input_2_1 = tf.nn.dropout(output_pool0, dropout)
    #     pre_sup_pool_1 = tf.matmul(input_2_1, layer_pool_1[1].weights['weights_0'])
    #     output_pool_1 = tf.nn.relu(tf.matmul(support1, pre_sup_pool_1) + layer_pool_1[1].weights['bias'])
    #     pool_all.append(output_pool_1)
    #
    #     input_2_2 = tf.nn.dropout(output_pool_1, dropout)
    #     pre_sup_pool_2 = tf.matmul(input_2_2, layer_pool_1[2].weights['weights_0'])
    #     output_pool_2 = tf.matmul(support1, pre_sup_pool_2) + layer_pool_1[2].weights['bias']
    #     pool_all.append(output_pool_2)
    #     assign_tensor = tf.nn.softmax(tf.layers.dense(tf.concat(pool_all, axis=1), self.assign_dim), axis=-1)
    #
    #     assign_x = tf.matmul(tf.transpose(assign_tensor), x_tensor_1)
    #     assign_a = tf.matmul(tf.matmul(tf.transpose(assign_tensor), self.adj), assign_tensor)
    #
    #     #第二个gcn
    #     In_1 = tf.constant(1, shape=[self.assign_dim], dtype=tf.float32)
    #     support1_1, D_1, D_11 = self.nomalize_adj(tf.add(assign_a, tf.diag(In_1)), self.assign_dim)
    #
    #
    #     input_3_0 = tf.nn.dropout(assign_x, dropout)
    #     pre_sup_gcn_0= tf.matmul(input_3_0, layer_gcn_1[3].weights['weights_0'])
    #     output_gcn_0 = tf.nn.relu(tf.matmul(support1_1, pre_sup_gcn_0) + layer_gcn_1[3].weights['bias'])
    #     x_all_gcn = [output_gcn_0]
    #
    #     input_3_1 = tf.nn.dropout(output_gcn_0, dropout)
    #     pre_sup_gcn_1 = tf.matmul(input_3_1, layer_gcn_1[4].weights['weights_0'])
    #     output_gcn_1 = tf.nn.relu(tf.matmul(support1_1, pre_sup_gcn_1) + layer_gcn_1[4].weights['bias'])
    #     x_all_gcn.append(output_gcn_1)
    #
    #     input_3_2 = tf.nn.dropout(output_gcn_1, dropout)
    #     pre_sup_gcn_2 = tf.matmul(input_3_2, layer_gcn_1[5].weights['weights_0'])
    #     output_gcn_2 = tf.matmul(support1_1, pre_sup_gcn_2) + layer_gcn_1[5].weights['bias']
    ...
    #     pre_0 = tf.nn.relu(tf.layers.dense(output, self.hidden_dim))
    #     output_att = tf.layers.dense(pre_0, self.embedding_dim)  # 输出
    #
    #     return output_att
...
# costruzione del modello con layers generici
        ...
        self.assign_tensor_f = tf.nn.softmax((tf.matmul(tf.concat(self.pool_all_f, axis=1), self.dis_variable['D_w1']) + self.dis_variable['D_b1']), axis=-1)
        ...
        pre_0 = tf.nn.softmax((tf.matmul(output, self.dis_variable['D_w2']) + self.dis_variable['D_b2']), axis=-1)
        self.output_att = tf.matmul(pre_0, self.dis_variable['D_w3']) + self.dis_variable['D_b3']

        self.output_att1 = self.predict(self.A, self.inputs_att, self.att_pool_mat, self.att_num_features_nonzero, self.num_nodes, self.assign_dim,
                                        self.nomalize_adj, self.gcn_layers, self.pool_layers, self.dis_variable, self.dropout)
        ...
        for i in range(len(self.support)):
            ...
            self.assign_tensor = tf.nn.softmax(
                (tf.matmul(tf.concat(self.pool_all, axis=1), self.dis_variable['D_w1']) + self.dis_variable['D_b1']),
                axis=-1)
            ...
            pre_0 = tf.nn.softmax((tf.matmul(output, self.dis_variable['D_w2']) + self.dis_variable['D_b2']), axis=-1)
            self.pre = tf.matmul(pre_0, self.dis_variable['D_w3']) + self.dis_variable['D_b3']
            #
...
# inizializzo loss e accuracy
    ...
    def predict(self, adj, input, pool_mat, num_features_nonzero, num_nodes, assign_dim, nomalize_adj, gcn_layers, pool_layers, dis_variable, dropout):
        ...
        assign_tensor = tf.nn.softmax(
            (tf.matmul(tf.concat(pool_all, axis=1), dis_variable['D_w1']) + dis_variable['D_b1']),
            axis=-1)
        ...
        pre_0 = tf.nn.softmax((tf.matmul(output, dis_variable['D_w2']) + dis_variable['D_b2']), axis=-1)
        output_att = tf.matmul(pre_0, dis_variable['D_w3']) + dis_variable['D_b3']
...
class GCNGraphs(BaseNet):
    ...
    def _build(self):
        ...
        self.dis_variable = {
            'D_w1': tf.Variable(tf.random_normal([self.num_hidden * 2 + self.assign_dim, self.assign_dim])),
            'D_b1': tf.Variable(tf.random_normal([self.assign_dim])),

            'D_w2': tf.Variable(tf.random_normal([self.num_nodes+self.assign_dim, self.hidden_dim])),
            'D_b2': tf.Variable(tf.random_normal([self.hidden_dim])),

            'D_w3': tf.Variable(tf.random_normal([self.hidden_dim, self.embedding_dim])),
            'D_b3': tf.Variable(tf.random_normal([self.embedding_dim])),
        }
