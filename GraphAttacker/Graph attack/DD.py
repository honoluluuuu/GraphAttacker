...
from utils import *
from densenet import GCNGraphs,GraphAttack
# Set random seed
import numpy as np
import copy
import random
...
flags = tf.app.flags
FLAGS = flags.FLAGS
flags.DEFINE_string('dataset', 'DD', 'which dataset to load')  # PROTEINS
...
flags.DEFINE_integer('epochs_c', 10, 'Number of epochs to train.')
...
# f_n = open("DD/DD_attack_graph.txt", 'a')
# a_g = []
# num = 0
# while num<99:
#     add = random.randint(0, len(graph)-1)
#     if add not in a_g:
#         a_g.append(add)
#         f_n.write(str(add)+'\n')
#         num +=1
...
att_g = []
f_n = open("DD/DD_attack_graph.txt", 'r')
...
y_train, y_val, y_test, idx_train, idx_val, idx_test, train_mask, val_mask, test_mask = get_splits_graphs_basic(num_supports, label[0], splits[0], splits[1], splits[2], idx[0])
...
while(attack_num<100):
    ...
    att_adj, att_fea, ori_support, att_pooling, ori_class, ori_label, att_label, att_mask, graph_link, att_size, att_fea_dense, graph_fea= att_graph1(
        attack_graph, graph, feats_matrix, labels, graph_size, a_size)
    ...
    for epoch in range(FLAGS.epochs):
        ...
        for i in range(train_batch):
            train_dict = build_dictionary_GCN(adj_batch[i], features[i], support[i], pool_mat1[i], label[i], train_mask,
                                              ori_support, att_fea, ori_label, att_mask, att_pooling, num_size[i],
                                              GCN_placeholders)
            train_dict.update({GCN_placeholders['dropout']: FLAGS.dropout})
            train_dict.update({GCN_placeholders['att_A']: att_adj})
            # Training step
            train_out = sess.run([network.opt_op, network.loss, network.accuracy, network.output, network.output_att],
                                 feed_dict=train_dict)
            #     train_out = sess.run([network.opt_op],feed_dict=train_dict)
            acc += train_out[2]

        for i in range(test_batch):
            test_dict = build_dictionary_GCN(adj_batch[train_batch + i],  features[train_batch + i],
                                             support[train_batch + i], pool_mat1[train_batch + i],
                                             label[train_batch + i], train_mask, ori_support, att_fea, ori_label,
                                             att_mask,
    ...
    #     att_adj1, att_fea1, att_support1, att_pooling1, ori_class1, ori_label1, att_label1, att_mask1, graph_link1, att_size1, att_fea_dense1, graph_fea1= att_graph(
    #         i, graph, feats_matrix, labels, graph_size, a_size)
    #     train_dict = build_dictionary_GCN(adj_batch[0], features[0], support[0], pool_mat[0], label[0], train_mask,
    #                                       att_support1, att_fea1, ori_label1, att_mask1, att_pooling1, GCN_placeholders)
    #     A_out = sess.run([network.accuracy_att, network.output_att],feed_dict=train_dict)
    #     A_out1 = sess.run([network.accuracy_att, network.output_att], feed_dict=train_dict)
    #     print(i,"  :",A_out[0])
    #     acc_s +=A_out[0]
    # acc_s = acc_s/num_graphs
    # print(acc_s)
    ...
    att_success = 0
    gcn_acc = 0
    save_dir = "result/DD_result-A-0.4-have.txt"
    ...
    while (att_success == 0 and att_time < 10):
        ...
        adj_save = 'DD/adj_DD-A-0.4-have/gen_PROTEINS_' + str(attack_graph) + '_' + str(
            att_time)
        ...
        Attgraph = GraphAttack(Att_placeholders, network, input_dim, featureless, adj_size1, num_supports, graph_size,
                               att_size, int(graph_size * 0.1), FLAGS.with_pooling)
        ...
        #     att_adj1, att_fea1, att_support1, att_pooling1, ori_class1, ori_label1, att_label1, att_mask1, graph_link1, att_size1, att_fea_dense1, graph_fea1 = att_graph1(
        #         i, graph, feats_matrix, labels, graph_size, a_size)
        #
        #     # att_adj, att_fea, ori_support, att_pooling, ori_class, ori_label, att_label, att_mask, graph_link, att_size, att_fea_dense, graph_fea = att_graph(
        #     #     attack_graph, graph, fea, labels, graph_size, a_size)
        #     train_dict = Att_dict = build_dictionary_Att(att_adj1, att_fea1, att_support1, att_pooling1, ori_label1, att_mask1, Att_placeholders)
        #     train_dict.update({Att_placeholders['dropout']: FLAGS.dropout})
        #     # A_out = sess.run([Attgraph.gen_adj_L], feed_dict=train_dict)
        #     # A_out = sess.run([Attgraph.output_ori], feed_dict=train_dict)
        #     A_out = sess.run([Attgraph.accuracy_att, Attgraph.output_ori, Attgraph.accuracy_ori, Attgraph.pre, Attgraph.gen_adj_L, Attgraph.adj], feed_dict=train_dict)
        #     print(i, "  :", A_out[0],"   ",A_out[2])
        #     # A_out1 = sess.run(
        #     #     [Attgraph.w0, Attgraph.v0,Attgraph.w1,Attgraph.v1,Attgraph.w2,Attgraph.v2, Attgraph.w3,Attgraph.v3], feed_dict=train_dict)
        #
        #     acc_s += A_out[0]
        #     acc_c += A_out[2]
        # acc_s = acc_s / len(graph)
        # acc_c = acc_c/ len(graph)
        # print(acc_s,"   ,",acc_c)
        #
        epoch = 0
        ...
        while (epoch < FLAGS.epochs_gan):
            ...
            for g_epoch in range(FLAGS.epochs_g):
                Att_dict = build_dictionary_Att(att_adj, att_fea, ori_support, att_pooling, ori_label, att_mask,
                                                Att_placeholders)
                Att_dict.update({Att_placeholders['dropout']: FLAGS.dropout})
                out_gen = sess.run([Attgraph.G_op, Attgraph.G_loss, Attgraph.gen_adj_L], feed_dict=Att_dict)
            ...
            for d_epoch in range(FLAGS.epochs_d):
                out_dis = sess.run([Attgraph.D_op, Attgraph.D_loss, Attgraph.D_real, Attgraph.D_fake],
                                   feed_dict=Att_dict)

            for c_epoch in range(FLAGS.epochs_c):
                Att_dict.update({Att_placeholders['labels']: att_label})
                out_cla = sess.run([Attgraph.C_op, Attgraph.C_loss, Attgraph.gen_adj_L, Attgraph.pre],
                                   feed_dict=Att_dict)
                ...
                att_acc = sess.run([network.accuracy_att, network.output_att, network.class_graph],
                                   feed_dict=train_dict)
            ...
            print("Epoch:", '%04d' % (epoch), "D_loss:", out_dis[1], "G_loss=", "{:.5f}".format(out_gen[1]), "C_loss=",
                  "{:.3f}".format(out_cla[1]))
            print("         Ori_acc=", acc_ori[0], "  Gen_acc=", acc_gen[0], "C_acc=", att_acc[0], "modify_num", eval_c)

            if acc_ori[2][0] != ori_class or eval_c == 0:
                gcn_acc = 0
                break
            if epoch > 1 and eval_c != 0:
                ...
                if acc_ori[2][0] == ori_class:
                    ...
                    if att_acc[0] == 0 and eval_c < graph_link*budget:#att_size*att_size * budget and eval_c<500:
                        print("第", attack_graph, "个图攻击，原类标为", ori_class, ",  攻击后类标为：", att_acc[2][0], " 原有连边数：",
                              graph_link,
                              "， 第", epoch, "个epoch攻击成功", "\n", "--------------****************---------------")
                        acc_node = 1
                        AML += eval_c
...
#     att_success = 0
#     gcn_acc = 0
#     save_dir = "result/DD_result-A-0.05-onehot.txt"
#     # save_dir = "result/PROTEINS_result-A-lr_c0.033-0.4-label.txt"
#     f = open(save_dir, 'a')
#
#     while (att_success == 0 and att_time < 10):
#         att_time += 1
#         # adj_save = 'PROTEINS/adj_PROTEINS-A-lr_c0.033-0.4-label/gen_PROTEINS_' + str(attack_graph) + '_' + str(att_time)
#         adj_save = 'DD/adj_DD-A-0.05-onehot/gen_PROTEINS_' + str(attack_graph) + '_' + str(
#             att_time)
#         print("第", attack_graph, "个图的第", att_time, "次攻击")
#
#         Att_placeholders = {
...
#
#                     # 当攻击后网络对原始gcn有攻击效果，并且修改连边数小于我们的预算，就停止攻击，输出结果。
#                     if att_acc[0] == 0 and eval_c < att_size*att_size * budget and eval_c<500:
#                         print("第", attack_graph, "个图攻击，原类标为", ori_class, ",  攻击后类标为：", att_acc[2][0], " 原有连边数：",
#                               graph_link,
#                               "， 第", epoch, "个epoch攻击成功", "\n", "--------------****************---------------")
#                         acc_node = 1
