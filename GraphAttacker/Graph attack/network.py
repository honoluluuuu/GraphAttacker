...
# costruzione del modello con layers generici
        ...
        self.d_fea = self.diffpool.layers[0](self.inputs, self.support, self.num_features_nonzero,0)
        ...
        self.pre = self.prediction1(self.gen_adj,self.activations, self.pool_mat, self.diffpool.var_l1, self.diffpool.var_l2,
                                   self.num_features_nonzero,self.dropout)



        self.activations_ori.append(self.inputs)
        hidden1 = self.diffpool.layers[0](self.activations_ori[-1], self.support, self.num_features_nonzero, 0)
        self.activations_ori.append(hidden1)

        hidden2 = self.diffpool.layers[1](self.activations_ori[-1], self.support, self.num_features_nonzero, 0)
        self.activations_ori.append(hidden2)
        self.pooling = tf.matmul(self.pool_mat, hidden2)
        self.output_ori = self.pooling

        self.a0 = self.adj_G - self.gen_adj_G
        self.a1 = tf.square(self.a0)
        self.a2 = tf.reduce_mean(self.a1)
...
# inizializzo loss e accuracy
    ...
    def prediction1(self, support, X, pool_mat,  w1, w2, num_features_nonzero, dropout):

        # self.activations.append(self.inputs)
        input_gcn = sparse_dropout(X[-1], 1 - dropout, num_features_nonzero)
        supports1 = list()
        pre_sup = tf.sparse_tensor_dense_matmul(input_gcn, w1)
        # In = tf.constant(1, shape=[support.shape[0]], dtype=tf.float32)
        In = tf.constant(1, shape=[self.num_nodes], dtype=tf.float32)
        support1, D, D1 = self.nomalize_adj(tf.add(support, tf.diag(In)))
        ...
        pooling = tf.matmul(pool_mat, outputs)
...
class GraphAttack(AttackNet):
    def __init__(self, placeholders,diffpool, input_dim, featureless,idx, num_graphs, num_nodes,att_size, with_pooling, **kwargs):
...
# costruzione del modello con layers generici
        ...
        for i in range(len(self.support)):
            ...
            hidden1 = self.gcn_layers[0](self.activations[i][-1], self.support[i], self.num_features_nonzero[i],i)
            self.x_all = [hidden1]
            self.activations[i].append(hidden1)

            hidden2 = self.gcn_layers[1](self.activations[i][-1], self.support[i], self.num_features_nonzero[i],i)
            self.x_all.append(hidden2)
            self.activations[i].append(hidden2)

            hidden3 = self.gcn_layers[2](self.activations[i][-1], self.support[i], self.num_features_nonzero[i],i)
            self.x_all.append(hidden3)
            self.d_fea = hidden1
            self.d_fea2 = hidden2

            self.x_tensor =tf.concat(self.x_all,axis=1)

            # self.x = self.gcn_layers[0](self.inputs[i], self.support[i], self.num_features_nonzero[i],i)
            #
            # self.x = self.gcn_layers[1](self.inputs[i], self.support[i], self.num_features_nonzero[i], i)


            self.pooling = tf.matmul(self.pool_mat[i], hidden3)
            ...
            # self.pooling = tf.matmul(self.pool_mat[i], hidden2)
            #
            self.out.append(self.pooling)
            # self.out.append(self.outputs)

            # self.activations_f.append([])
            # self.activations_f[i].append(self.inputs[i])
            # self.pre = self.prediction1(self.adj[i], self.activations_f[i], self.pool_mat[i], self.layers[0].weights['weights_0'],
            #                             self.layers[1].weights['weights_0'],
            #                             self.num_features_nonzero[i], self.dropout)
            # self.out_f.append(self.pre)
        ...
        # # self.activations_ori.append(self.inputs)
        # hidden11 = self.layers[0](self.activations_f[-1], self.att_adj, self.att_num_features_nonzero, 0)
        # self.activations_f.append(hidden11)
        # self.d_fea2_att = self.layers[1](self.activations_f[-1], self.att_adj, self.att_num_features_nonzero, 0)
        # #
        # self.pool1_att = self.layers[2](self.activations_pool_f[-1], self.att_adj, self.att_num_features_nonzero, i)
        # self.activations_pool_f.append(self.pool1_att)
        # self.pool2_att = self.layers[3](self.activations_pool_f[-1], self.att_adj, self.att_num_features_nonzero, i)
        # self.output_att = tf.matmul(tf.transpose(self.pool2_att), self.d_fea2_att)

        self.activations_f.append(self.inputs_att)
        # self.activations_ori.append(self.inputs)
        hidden11 = self.gcn_layers[0](self.activations_f[-1], self.att_adj, self.att_num_features_nonzero, 0)
        self.activations_f.append(hidden11)

        hidden22 = self.gcn_layers[1](self.activations_f[-1], self.att_adj, self.att_num_features_nonzero, 0)
        self.activations_f.append(hidden22)

        hidden33 = self.gcn_layers[2](self.activations_f[-1], self.att_adj, self.att_num_features_nonzero, 0)
        self.output_att = tf.matmul(self.att_pool_mat, hidden33)
        # self.output_att = self.pooling
...
# salvo per comodità le variabili del modello invece che tenerle solo in tf.GraphKeys.GlOBALVARIABLES
        ...
        self.var_l1 = self.gcn_layers[0].weights['weights_0']
        self.var_l2 = self.gcn_layers[1].weights['weights_0']
# inizializzo loss e accuracy
        self._loss()
        self._accuracy()
...
class GCNGraphs(BaseNet):
    ...
    def _build(self):
        ...
        self.gcn_layers.append(ConvolutionalLayer(input_dim=self.hidden_dim,output_dim=self.embedding_dim,placeholders=self.placeholders,activation=lambda x: x,
                                              dropout=True,sparse_inputs=False,featureless = self.featureless))
        # self.gcn_layers.append(ConvolutionalLayer(input_dim=self.hidden_dim,output_dim=2,placeholders=self.placeholders,activation=tf.nn.relu,
        #                                       dropout=True,sparse_inputs=False,featureless = self.featureless))



        # self.layers.append(ConvolutionalLayer(input_dim=self.input_dim,output_dim=64,placeholders=self.placeholders,activation=tf.nn.relu,
        #                                       dropout=True,sparse_inputs=True,featureless = self.featureless))
        #
        #
        # self.layers.append(ConvolutionalLayer(input_dim=64,
        #                                     output_dim=2,
        #                                     placeholders=self.placeholders,
        #                                     activation=lambda x: x,
        #                                     dropout=True,
        ...
        #
        #
        # # self.layers.append(ConvolutionalLayer(input_dim=256,
        # #                                     output_dim=90,
        # #                                     placeholders=self.placeholders,
        # #                                     activation=tf.nn.relu, #lambda x: x
        # #                                     dropout=True,
        # #                                     sparse_inputs=False,
        # #                                     featureless = False))
        # self.layers.append(ConvolutionalLayer(input_dim=64,
        ...
        #                                     featureless = False))
        # #gcn2
        # self.layers.append(ConvolutionalLayer_1(input_dim=64,
        #                                     output_dim=64,
        #                                     placeholders=self.placeholders,
        #                                     activation=tf.nn.relu,
        #                                     dropout=True,
        ...
        #
        #
        # self.layers.append(ConvolutionalLayer_1(input_dim=25,
        #                                     output_dim=1,
        #                                     placeholders=self.placeholders,
        #                                     activation=lambda x: x,
        #                                     dropout=True,
        ...
        #                                     featureless = False))
        self.dis_variable = {
            'D_w1': tf.Variable(tf.random_normal([32, self.output_dim])),
            'D_b1': tf.Variable(tf.random_normal([self.output_dim])),
            # 'D_w2': tf.Variable(tf.random_normal([self.num_hidden, 1])),
            # 'D_b2': tf.Variable(tf.random_normal([1])),
        }
